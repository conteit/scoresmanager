...
import it.conteit.scoresmanager.gui.valiators.UniqueInScoresListValidator;
...
public class ScoreNameInputDialog extends AbstractApplicationDialog {
	...
	private IDay day;
	private IScore result = null;
	private boolean penality;
	
	private UniqueInScoresListValidator val;

	public ScoreNameInputDialog(){
		this((Frame) null, null, false);
	}
	
	public ScoreNameInputDialog(Dialog owner, IDay day, boolean penality) {
		super(owner);
		this.penality = penality;
		createImportGrestInputDialog(day);
	}

	public ScoreNameInputDialog(Frame owner, IDay day, boolean penality) {
		super(owner);
		this.penality = penality;
		createImportGrestInputDialog(day);
	}
	 ...
	 * @param serializer
	 */
	public void createImportGrestInputDialog(IDay day) {
		setTitle("Add new score");
		...
		{
			JLabel lblTheImportedGrest = new JLabel("Insert new score d name:");
		...
		val = new UniqueInScoresListValidator(day, inputField);
		addValidator(val);
		check();
	}
	...
	protected void ok_action(ActionEvent e) {
		try {
			if (result == null) {
				if(penality){
					result = Penality.create(inputField.getText(), day.teamCount());
				} else {
					result = Partial.create(inputField.getText(), day.teamCount());	
				}
			} else {
	...
	public IScore getResult() {
		return result;
	}

	public void setScore(IScore score) {
		result = score;
		inputField.setText(score.getDescription());
		val.setOldValue(score.getDescription());
	...
	public static IScore showDialog(IDay day, boolean penality) {
		ScoreNameInputDialog dialog = new ScoreNameInputDialog((Frame) null, day, penality);
		dialog.setVisible(true);

		return dialog.getResult();
	}

	public static IScore showDialog(IDay day, IScore score) {
		ScoreNameInputDialog dialog = new ScoreNameInputDialog((Frame) null, day, false);
